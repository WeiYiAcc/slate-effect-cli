import { n as CompactionError, o as ContextCompactor } from "./ContextCompactor-CTyx9juD.mjs";
import { l as ToolExecutionClassValue, o as RunStepHook } from "./DurableStep-BvxMOgOi.mjs";
import { AgentInputError } from "@effect-agent/core/AgentError";
import { AgentPolicy } from "@effect-agent/core/AgentPolicy";
import { AgentId, DelegationId, ReceiptId, RunId, SubmissionId, ThreadId, ToolCallId, TurnId } from "@effect-agent/core/Identifiers";
import { DelegationDepth, SubagentBudgetReservation, SubagentParentLink, ToolExecutionKind } from "@effect-agent/core/SubagentContract";
import { ModelCallUsage } from "@effect-agent/core/Usage";
...
declare namespace RunOptions_d_exports {
  export { ChildEstablishDenied, ChildEstablishSettled, ChildEstablishStatus, ChildEstablishWaiting, CommandDrainPolicy, CurrentToolFailureObserver, ModelToolFailure, PreparedRunContext, ProgrammaticPreflightFailure, ProgrammaticToolFailure, RunApprovalDecision, RunApprovalHook, RunApprovalRequest, RunBudgetHook, RunBufferLimits, RunCompactionCommit, RunContextHook, RunContextPreparation, RunContextPreparationError, RunContextPreparationPassthrough, RunContextRequest, RunCostEstimate, RunCostEstimateRequest, RunCostEstimateValue, RunCostEstimator, RunDurabilityHook, RunInputCommand, RunInputHook, RunOptions, RunOptionsError, RunOptionsRequirements, RunResumeUsage, RunResumeUsageSchema, RunSchedulingHook, RunSchedulingOverride, RunSubagentChildIdentity, RunSubagentDigests, RunSubagentEstablishRequest, RunSubagentHook, RunSubagentJoinRequest, RunToolAuthorization, RunToolAuthorizationDecision, RunToolAuthorizationHook, RunToolAuthorizationRequest, RunToolCallDescriptor, RunToolFailureObserver, RunTransientContextHook, RunTurnResponseCommit, RunTurnResume, RunTurnResumeCall, RunTurnResumeSettledCall, RunTurnResumeSettledCallSchema, RunTurnUsage, RunUsageDelta, ToolFailureObservation, toolFailureObserverLayer };
}
...
interface ModelToolFailure extends ToolFailureIdentity {
...
type ProgrammaticToolFailure = ProgrammaticDeclaredFailure | ProgrammaticHandlerFailure | ProgrammaticDiagnosticFailure;
/** A programmatic invocation was rejected before its Handler started. No inner identity exists. */
interface ProgrammaticPreflightFailure extends ToolFailureIdentity {
...
type ToolFailureObservation = ModelToolFailure | ProgrammaticToolFailure | ProgrammaticPreflightFailure;
/**
 ...
 */
interface RunToolFailureObserver {
...
declare const CurrentToolFailureObserver: Context.Reference<RunToolFailureObserver | undefined>;
/** The sole installation seam, shared by ephemeral Runs and durable platform options. */
declare const toolFailureObserverLayer: (observer: RunToolFailureObserver) => Layer.Layer<never>;
/** Number of queued inputs consumed at one documented Turn seam. */
declare const CommandDrainPolicy: Schema.Literals<readonly ["one", "all"]>;
type CommandDrainPolicy = typeof CommandDrainPolicy.Type;
/** Engine-normalized input command. Capability packages may adapt richer audit records to it. */
interface RunInputCommand {
 ...
 */
interface RunInputHook<Error = never, Requirements = never> {
  readonly start?: (() => Effect.Effect<void, Error, Requirements>) | undefined;
  readonly drain: (policy: CommandDrainPolicy) => Effect.Effect<ReadonlyArray<RunInputCommand>, Error, Requirements>;
...
interface RunApprovalHook<Error = never, Requirements = never> {
  readonly request: (request: RunApprovalRequest) => Effect.Effect<RunApprovalDecision, Error, Requirements>;
}
/** Input passed to ordered context transformation and compaction adapters. */
interface RunContextRequest {
  readonly threadId: ThreadId;
  readonly runId: RunId;
  readonly turnId: TurnId;
...
interface RunContextHook<Error = never, Requirements = never> {
  readonly prepare: (request: RunContextRequest) => Effect.Effect<PreparedRunContext, Error, Requirements>;
}
/**
...
interface RunTransientContextHook<Error = never, Requirements = never> {
  readonly load: (request: RunContextRequest) => Effect.Effect<Prompt.RawInput, Error, Requirements>;
}
/** Context service failures retain their concrete tags and structured fields. */
type RunContextPreparationError = AgentInputError | MemoryRecallError | CompactionError;
/**
...
type RunCostEstimateValue = number | RunCostEstimate;
/** Model identity presented beside the legacy raw-usage estimator argument. */
interface RunCostEstimateRequest {
 ...
 */
type RunCostEstimator<Error = never, Requirements = never> = (usage: Response.Usage, request: RunCostEstimateRequest) => Effect.Effect<RunCostEstimateValue, Error, Requirements>;
/**
 ...
 */
interface RunBudgetHook<Error = never, Requirements = never> {
  /**
   ...
   */
  readonly guard: <A, E, R>(effect: Effect.Effect<A, E, R>) => Effect.Effect<A, E | Error, R | Requirements>;
 ...
 */
interface RunToolCallDescriptor {
  readonly toolCallId: ToolCallId;
  readonly toolName: string;
 ...
 */
interface RunToolAuthorizationRequest {
  readonly threadId: ThreadId;
  readonly runId: RunId;
  readonly turnId: TurnId;
  readonly turn: number;
...
interface RunToolAuthorizationHook<Error = never, Requirements = never> {
  readonly authorize: (request: RunToolAuthorizationRequest) => Effect.Effect<RunToolAuthorizationDecision, Error, Requirements>;
}
declare const RunContextPreparation_base: Context.ServiceClass<RunContextPreparation, "@effect-agent/engine/RunContextPreparation", {
 ...
 */
declare class RunContextPreparation extends RunContextPreparation_base {}
/** Explicit no-preparer Layer used by compatible runtime assemblies. */
declare const RunContextPreparationPassthrough: Layer.Layer<RunContextPreparation>;
declare const RunToolAuthorization_base: Context.ServiceClass<RunToolAuthorization, "@effect-agent/engine/RunToolAuthorization", RunToolAuthorizationHook<never, never>>;
/**
 ...
 */
declare class RunToolAuthorization extends RunToolAuthorization_base {
  /** Explicit compatibility policy: Tool execution requires no additional host authorization. */
  static readonly allowAll: Layer.Layer<RunToolAuthorization>;
}
/**
 ...
 */
interface RunTurnResponseCommit {
  readonly turn: number;
  readonly turnId: TurnId;
 ...
 */
interface RunDurabilityHook<Error = never, Requirements = never> {
  /**
   ...
   */
  readonly reservePolicyUsage?: ((usage: Pick<RunPolicyUsage, "programmaticToolCalls" | "finalizationUsed">) => Effect.Effect<void, Error, Requirements>) | undefined;
  /** After validated staged response events are emitted, before approval preflight. */
  readonly commitResponse: (commit: RunTurnResponseCommit) => Effect.Effect<void, Error, Requirements>;
  /** After every approval and host authorization resolved allowed, before any handler starts. */
  readonly prepareToolCalls: (calls: ReadonlyArray<RunToolCallDescriptor>) => Effect.Effect<void, Error, Requirements>;
   ...
   */
  readonly commitCompaction: (commit: RunCompactionCommit) => Effect.Effect<void, Error, Requirements>;
  /**
 ...
 * one existing child.
 */
interface RunSubagentEstablishRequest {
...
interface RunSubagentChildIdentity {
...
interface ChildEstablishWaiting extends RunSubagentChildIdentity {
 ...
 */
interface ChildEstablishSettled extends RunSubagentChildIdentity {
 ...
 */
interface ChildEstablishDenied {
  readonly _tag: "denied";
  readonly errorTag: string;
  readonly message: string;
}
/** Result of one idempotent durable child establishment attempt. */
type ChildEstablishStatus = ChildEstablishWaiting | ChildEstablishSettled | ChildEstablishDenied;
/**
 ...
 * transitions.
 */
interface RunSubagentJoinRequest {
  readonly toolCallId: ToolCallId;
 ...
 */
interface RunSubagentHook<Error = never, Requirements = never> {
  readonly establish: (request: RunSubagentEstablishRequest) => Effect.Effect<ChildEstablishStatus, Error, Requirements>;
 ...
 */
declare const RunTurnResumeSettledCallSchema: Schema.Struct<{
...
type RunTurnResumeSettledCall = typeof RunTurnResumeSettledCallSchema.Type;
/**
 ...
 */
declare const RunResumeUsageSchema: Schema.Struct<{
  readonly committedTurns: Schema.Natural;
  readonly toolCalls: Schema.Natural;
  readonly programmaticToolCalls: Schema.Natural;
  readonly consecutiveToolFailures: Schema.Natural;
  readonly finalizationUsed: Schema.Boolean;
...
type RunResumeUsage = typeof RunResumeUsageSchema.Type;
/**
...
declare const RunSchedulingOverride: Schema.Union<readonly [Schema.Struct<{
  readonly mode: Schema.Literal<"bounded">;
  readonly concurrency: Schema.Int;
}>, Schema.Struct<{
  readonly mode: Schema.Literal<"sequential">;
}>]>;
type RunSchedulingOverride = typeof RunSchedulingOverride.Type;
/** Dependency-neutral scheduling policy surrounding native Effect AI Tool handlers. */
interface RunSchedulingHook {
...
interface RunOptions<HookError = never, HookRequirements = never> {
   ...
   */
  readonly onHistory?: ((history: Prompt.Prompt) => Effect.Effect<void, HookError, HookRequirements>) | undefined;
}
type RunOptionsError<Options> = Options extends RunOptions<infer Error, infer _Requirements> ? Error : never;
type RunOptionsRequirements<Options> = Options extends RunOptions<infer _Error, infer Requirements> ? Requirements : never;
//#endregion
export { RunTurnResumeSettledCallSchema as $, RunOptionsError as A, RunSubagentHook as B, RunCostEstimateRequest as C, RunInputCommand as D, RunDurabilityHook as E, RunSchedulingHook as F, RunToolAuthorizationRequest as G, RunToolAuthorization as H, RunSchedulingOverride as I, RunTransientContextHook as J, RunToolCallDescriptor as K, RunSubagentChildIdentity as L, RunOptions_d_exports as M, RunResumeUsage as N, RunInputHook as O, RunResumeUsageSchema as P, RunTurnResumeSettledCall as Q, RunSubagentDigests as R, RunCostEstimate as S, RunCostEstimator as T, RunToolAuthorizationDecision as U, RunSubagentJoinRequest as V, RunToolAuthorizationHook as W, RunTurnResume as X, RunTurnResponseCommit as Y, RunTurnResumeCall as Z, RunContextHook as _, CommandDrainPolicy as a, RunContextPreparationPassthrough as b, PreparedRunContext as c, RunApprovalDecision as d, RunTurnUsage as et, RunApprovalHook as f, RunCompactionCommit as g, RunBufferLimits as h, ChildEstablishWaiting as i, RunOptionsRequirements as j, RunOptions as k, ProgrammaticPreflightFailure as l, RunBudgetHook as m, ChildEstablishSettled as n, ToolFailureObservation as nt, CurrentToolFailureObserver as o, RunApprovalRequest as p, RunToolFailureObserver as q, ChildEstablishStatus as r, toolFailureObserverLayer as rt, ModelToolFailure as s, ChildEstablishDenied as t, RunUsageDelta as tt, ProgrammaticToolFailure as u, RunContextPreparation as v, RunCostEstimateValue as w, RunContextRequest as x, RunContextPreparationError as y, RunSubagentEstablishRequest as z };
//# sourceMappingURL=RunOptions-BTJmqzIp.d.mts.map