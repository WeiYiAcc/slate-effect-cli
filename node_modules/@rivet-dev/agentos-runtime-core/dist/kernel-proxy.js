...
export class NativeSidecarKernelProxy {
    ...
    async findListenerAsync(request) {
        ...
        const pending = existing?.pending ??
            this.refreshSocketLookup(this.listenerLookups, key, () => this.client.findListener(this.session, this.vm, request));
        if (!existing?.pending) {
            this.listenerLookups.set(key, {
    ...
    async startTrackedProcess(entry) {
        ...
        this.signalRefreshes.set(entry.pid, this.refreshSignalState(entry).catch(() => { }));
        void this.flushPendingStdin(entry).catch((error) => {
            this.handleBackgroundProcessError(entry, error);
        });
        void this.closeTrackedStdin(entry).catch((error) => {
    ...
    async signalProcess(entry, signal) {
        ...
        let timedOut = false;
        const killPromise = this.client.killProcess(this.session, this.vm, entry.processId, sidecarSignal);
        try {
            await Promise.race([
                killPromise,
            ...
            const hasGuestHandler = this.signalStates.get(entry.pid)?.handlers.has(signal) ?? false;
            if (entry.exitCode === null &&
                !hasGuestHandler &&
