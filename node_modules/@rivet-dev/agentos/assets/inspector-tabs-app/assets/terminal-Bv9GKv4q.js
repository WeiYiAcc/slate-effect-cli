...
For tests, pass a Ghostty instance directly:
  ...
  const term = new Terminal({ ghostty });`);return m}const xA=80,pA=24,TA=5e3,_=8,dA=256*1024,$=1024*1024;let AA;function mA(){return AA??(AA=nA()),AA}function bA(){const[Q,A]=U.useState({ready:!1,error:null});return U.useEffect(()=>{let B=!1;return mA().then(()=>!B&&A({ready:!0,error:null}),g=>!B&&A({ready:!1,error:g})),()=>{B=!0}},[]),Q}class fA{constructor(){p(this,"writers",new Map);p(this,"pending",new Map)}push(A,B,g){var D;const E=this.writers.get(A);if(E){E(g);return}const C=this.pending.get(A)??[];C.push({seq:B,bytes:g});let I=C.reduce((i,w)=>i+w.bytes.length,0);for(;I>dA&&C.length>1;)I-=((D=C.shift())==null?void 0:D.bytes.length)??0;this.pending.set(A,C)}attach(A,B,g=0){this.writers.set(A,B);const E=this.pending.get(A);if(E){this.pending.delete(A);for(const C of E)C.seq>g&&B(C.bytes)}return()=>{this.writers.get(A)===B&&this.writers.delete(A)}}forget(A){this.writers.delete(A),this.pending.delete(A)}}class zA{constructor(A,B){p(this,"queues",new Map);this.send=A,this.onError=B}write(A,B){const g=this.queues.get(A)??{pending:"",flushing:!1};if(g.pending.length+B.length>$){this.onError(new Error(`Pending terminal input exceeds ${$} characters; raise MAX_PENDING_INPUT_CHARS in tabs/terminal.tsx`));return}g.pending+=B,this.queues.set(A,g),g.flushing||this.flush(A,g)}async flush(A,B){B.flushing=!0;try{for(;B.pending;){const g=B.pending;B.pending="",await this.send(A,g)}}catch(g){B.pending="",this.onError(g)}finally{B.flushing=!1}}forget(A){this.queues.delete(A)}}function VA(){return document.documentElement.classList.contains("dark")?{background:"#0b0e14",foreground:"#c7d0e0",cursor:"#c7d0e0"}:{background:"#ffffff",foreground:"#1f2328",cursor:"#1f2328"}}const uA=/\x1b\[\d+;\d+R/g;function WA({shellId:Q,active:A,router:B,onInput:g,onResize:E,onError:C}){const I=U.useRef(null),D=U.useRef(null),i=U.useRef(null),w=U.useRef(g),M=U.useRef(E),N=U.useRef(C);return w.current=g,M.current=E,N.current=C,U.useEffect(()=>{const k=I.current;if(!k)return;const s=new YA({cursorBlink:!0,fontFamily:'ui-monospace, SFMono-Regular, Menlo, Consolas, "Liberation Mono", monospace',fontSize:13,scrollback:TA,theme:VA()}),h=new OA;s.loadAddon(h),s.open(k),D.current=s,i.current=h,h.fit(),h.observeResize(),s.onData(a=>{const F=a.replace(uA,"");F&&w.current(F)}),s.onResize(({cols:a,rows:F})=>M.current(a,F));let G=null,K=!1;const t=a=>s.write(a);return O.shellSnapshot(Q,"screen").then(a=>{K||(a!=null&&a.data&&s.write(a.data),G=B.attach(Q,t,(a==null?void 0:a.seq)??0))},a=>{K||(G=B.attach(Q,t),N.current(a))}),()=>{K=!0,G==null||G(),h.dispose(),s.dispose(),D.current=null,i.current=null}},[Q,B]),U.useEffect(()=>{var k,s;A&&((k=i.current)==null||k.fit(),(s=D.current)==null||s.focus())},[A]),J.jsx("div",{ref:I,className:"agentos-terminal-pane absolute inset-0",style:{visibility:A?"visible":"hidden"}})}function _A({actorId:Q}){return J.jsx(hA,{actorId:Q,note:"VM not booted.",actionLabel:"Boot the VM and open a terminal",children:J.jsx(ZA,{actorId:Q})})}function ZA({actorId:Q}){var S;const A=bA(),g=U.useRef(new fA).current,E=DA(),C=oA(T(Q)),[I,D]=U.useState(null),i=U.useMemo(()=>(C.data??[]).map((e,c)=>({shellId:e.shellId,title:`shell ${c+1}`})),[C.data]),[w,M]=U.useState(null),N=w&&i.some(e=>e.shellId===w)?w:((S=i[i.length-1])==null?void 0:S.shellId)??null,k=U.useCallback(()=>E.invalidateQueries({queryKey:T(Q).queryKey}),[Q,E]),s=U.useCallback(e=>{var c;g.forget(e),(c=a.current)==null||c.forget(e),E.setQueryData(T(Q).queryKey,q=>q&&q.filter(gA=>gA.shellId!==e)),k()},[Q,E,k,g]),G=iA().useEvent;G("shellData",e=>{const c=e;if(!c||typeof c.shellId!="string")return;const q=wA(c.data);q.length>0&&g.push(c.shellId,c.seq??0,q)}),G("shellExit",e=>{const c=e;!c||typeof c.shellId!="string"||s(c.shellId)});const K=j({mutationFn:()=>O.openShell(xA,pA),onSuccess:({shellId:e})=>{D(null),E.setQueryData(T(Q).queryKey,c=>c&&[...c,{shellId:e,openedAt:Date.now()}]),M(e),k()},onError:D}),t=j({mutationFn:e=>O.closeShell(e),onMutate:s,onError:D}),a=U.useRef(null);a.current??(a.current=new zA(O.writeShell,D));const F=a.current,L=U.useCallback((e,c)=>F.write(e,c),[F]),x=U.useCallback((e,c,q)=>{O.resizeShell(e,c,q).catch(D)},[]);if(A.error)return J.jsx(f,{children:J.jsxs("div",{className:"flex max-w-sm flex-col items-center gap-2",children:[J.jsx("span",{children:"Terminal renderer failed to load."}),J.jsx(Z,{error:A.error})]})});if(C.data===null)return J.jsx(sA,{action:"listShells"});const l=i.length>=_;return J.jsxs("div",{className:"flex h-full min-h-0 flex-col",children:[J.jsxs("div",{className:"flex items-center gap-1 border-b px-2 py-1.5",children:[i.map(e=>J.jsxs("div",{onClick:()=>M(e.shellId),className:MA("flex cursor-pointer items-center gap-1.5 rounded px-2 py-1 text-xs transition-colors",e.shellId===N?"bg-muted text-foreground":"text-muted-foreground hover:bg-muted/50"),children:[J.jsx("span",{className:"font-mono",children:e.title}),J.jsx("button",{type:"button",title:"Close shell",onClick:c=>{c.stopPropagation(),t.mutate(e.shellId)},className:"text-muted-foreground/60 transition-colors hover:text-foreground",children:"×"})]},e.shellId)),J.jsx(tA,{title:l?`Limit of ${_} shells reached`:"New shell",onClick:()=>K.mutate(),disabled:!A.ready||K.isPending||l,children:J.jsx(NA,{})}),J.jsx("div",{className:"ml-auto",children:J.jsx(kA,{actorId:Q})})]}),I?J.jsx(Z,{error:I}):null,J.jsx("div",{className:"relative min-h-0 flex-1 bg-background",children:A.ready?i.length===0?J.jsx(f,{children:"No shells open — click + to start one."}):i.map(e=>J.jsx(WA,{shellId:e.shellId,active:e.shellId===N,router:g,onInput:c=>L(e.shellId,c),onResize:(c,q)=>x(e.shellId,c,q),onError:D},e.shellId)):J.jsx(f,{children:"Loading terminal renderer…"})})]})}export{_A as TerminalTabConnected};
