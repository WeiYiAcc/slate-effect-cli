    ...
    describe(name, () => {
        ...
        describe("core", () => {
            ...
            test("readDir on missing directory throws ENOENT", async () => {
                if (capabilities.allowMissingDirReadAsEmpty) {
                    await expect(fs.readDir("/nonexistent-dir")).resolves.toEqual([]);
                    return;
                }
                const err = await fs.readDir("/nonexistent-dir").catch((e) => e);
                expect(err).toBeInstanceOf(Error);
                expect(hasErrorCode(err, "ENOENT")).toBe(true);
            });
            ...
            test("rename of missing source throws ENOENT", async () => {
                if (capabilities.allowMissingSourceRenameNoop) {
                    await expect(fs.rename("/nonexistent.txt", "/dst.txt")).resolves.toBeUndefined();
                    return;
                }
            ...
            if (capabilities.removeDir) {
                test("rename a directory with children moves all children", async () => {
        ...
        if (capabilities.symlinks) {
            describe("symlinks", () => {
                ...
                test("symlink loop throws ELOOP or EINVAL", async () => {
                    ...
                    expect(err).toBeInstanceOf(Error);
                    expect(hasErrorCode(err, "ELOOP") ||
                        (capabilities.allowSymlinkLoopErrnoFallback &&
                            hasErrorCode(err, "EINVAL"))).toBe(true);
                });
                ...
                test("symlink on an existing path throws EEXIST", async () => {
                    ...
                    if (capabilities.allowSymlinkOverwrite) {
                        await expect(fs.symlink("/other.txt", "/existing.txt")).resolves.toBeUndefined();
                        return;
                    }
                    const err = await fs
                        ...
                        .catch((e) => e);
                    expect(err).toBeInstanceOf(Error);
                    expect(hasErrorCode(err, "EEXIST")).toBe(true);
                });
            });
        }
        // ---------------------------------------------------------------
        ...
        if (capabilities.hardLinks) {
            describe("hardLinks", () => {
                ...
                test("link on a directory throws EPERM or an error", async () => {
                    ...
                    expect(err).toBeInstanceOf(Error);
                    expect(hasErrorCode(err, "EPERM")).toBe(true);
                });
            });
        }
        // ---------------------------------------------------------------
            ...
            describe("permissions", () => {
                ...
                test("chown changes uid/gid", async () => {
                    ...
                    if (result instanceof Error) {
                        expect(hasErrorCode(result, "EPERM") || hasErrorCode(result, "ENOSYS")).toBe(true);
                    }
                    else {
        ...
        if (capabilities.mkdir) {
            describe("mkdir", () => {
                ...
                test("mkdir without recursive throws ENOENT when parent is missing", async () => {
                    if (capabilities.allowMkdirWithoutRecursiveParentAutoCreate) {
                        await expect(fs.mkdir("/x/y/z", { recursive: false })).resolves.toBeUndefined();
                        expect(await fs.exists("/x/y/z")).toBe(true);
                        return;
                    }
                    const err = await fs
                        .mkdir("/x/y/z", { recursive: false })
                        .catch((e) => e);
                    expect(err).toBeInstanceOf(Error);
                    expect(hasErrorCode(err, "ENOENT")).toBe(true);
                });
        ...
        if (capabilities.removeDir) {
            describe("removeDir", () => {
                ...
                test("removeDir on non-empty directory throws ENOTEMPTY", async () => {
                    ...
                    if (capabilities.allowRemoveDirNonEmptyRecursiveDelete) {
                        await expect(fs.removeDir("/nonempty")).resolves.toBeUndefined();
                        expect(await fs.exists("/nonempty")).toBe(false);
                        return;
                    }
                    const err = await fs.removeDir("/nonempty").catch((e) => e);
                    expect(err).toBeInstanceOf(Error);
                    expect(hasErrorCode(err, "ENOTEMPTY")).toBe(true);
                });
            });
        }
    });
