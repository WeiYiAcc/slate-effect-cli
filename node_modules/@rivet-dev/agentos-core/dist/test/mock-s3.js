...
function verifySigV4(options) {
    const parsed = parseAuthorization(normalizeHeaderValue(options.headers.authorization));
    if (parsed.accessKeyId !== options.accessKeyId) {
        throw new Error(`unexpected access key id: ${parsed.accessKeyId} (expected ${options.accessKeyId})`);
    }
    if (parsed.service !== "s3" || parsed.terminal !== "aws4_request") {
        throw new Error(`unexpected credential scope: ${parsed.service}/${parsed.terminal}`);
    }
    const payloadHash = normalizeHeaderValue(options.headers["x-amz-content-sha256"]);
    if (!payloadHash) {
    ...
    if (payloadHash !== "UNSIGNED-PAYLOAD" &&
        ...
        throw new Error(`payload hash mismatch: expected ${expectedPayloadHash}, got ${payloadHash}`);
    }
    ...
    if (!xAmzDate.startsWith(parsed.date)) {
        throw new Error(`x-amz-date ${xAmzDate} does not match credential date ${parsed.date}`);
    }
    ...
    const signingKey = hmac(hmac(hmac(hmac(`AWS4${options.secretAccessKey}`, parsed.date), parsed.region), parsed.service), parsed.terminal);
    ...
    if (actual.length !== expected.length || !timingSafeEqual(actual, expected)) {
        throw new Error(`signature mismatch: expected ${expectedSignature}, got ${parsed.signature}`);
    }
}
function xmlError(code, message) {
    return Buffer.from(`<Error><Code>${code}</Code><Message>${message}</Message></Error>`);
}
export async function startMockS3Server() {
    ...
    const server = createServer(async (request, response) => {
        ...
        if (!SUPPORTED_METHODS.has(method)) {
            fail(501, "NotImplemented", `unsupported S3 method ${method}; update the local harness before adding new protocol calls`);
            return;
        }
        const expectedOperationId = method === "GET"
            ? "GetObject"
            : method === "PUT"
                ? "PutObject"
                : "DeleteObject";
        if (query && query !== `x-id=${expectedOperationId}`) {
            fail(501, "NotImplemented", `unsupported S3 query string '${query}'; only x-id=${expectedOperationId} is allowed for ${method}`);
            return;
        }
        if (bucket !== DEFAULT_BUCKET || !key) {
            fail(400, "InvalidURI", `expected path-style /${DEFAULT_BUCKET}/<key> request, got ${decodedPath}`);
            return;
        }
        const chunks = [];
            ...
            return new Promise((resolve, reject) => {
                server.close((error) => {
                    ...
                    if (protocolViolations.length > 0) {
                        reject(new Error(`Strict mock S3 server saw unsupported protocol usage:\n- ${protocolViolations.join("\n- ")}`));
                        return;
                    }
                    resolve();
                });
