...
function decodePath(rootDir, rawPath) {
...
function mapProcessPath(rootDir, value) {
...
function appendLog(proc, stream, chunk) {
    proc.sequence += 1;
    proc.logs.push({
...
async function waitForProcessExit(proc, timeoutMs = 2_000) {
...
async function runCommand(request) {
    ...
    return await new Promise((resolveRun) => {
        ...
        let settled = false;
        let timedOut = false;
        const child = spawn(request.command, request.args?.map((value) => mapProcessPath(request.rootDir, value)) ?? [], {
...
export async function startMockSandboxAgent() {
    ...
    const server = createServer(async (request, response) => {
        try {
            ...
            if (method === "POST" && url.pathname === "/v1/processes/run") {
                ...
                json(response, 200, await runCommand({
                    ...requestBody,
                    rootDir,
            ...
            if (method === "POST" && url.pathname === "/v1/processes") {
                const requestBody = JSON.parse((await readBody(request)).toString("utf8"));
                const child = spawn(requestBody.command, requestBody.args?.map((value) => mapProcessPath(rootDir, value)) ?? [], {
                    cwd: requestBody.cwd
                ...
                child.stdout.on("data", (chunk) => appendLog(proc, "stdout", chunk));
                child.stderr.on("data", (chunk) => appendLog(proc, "stderr", chunk));
                child.on("close", (code) => {
                    proc.status = "exited";
                    proc.exitCode = code;
            ...
            problem(response, 400, error instanceof Error ? error.message : String(error));
        }
    });
    ...
    return {
        ...
        client,
        rootDir,
        path: (...segments) => resolve(rootDir, ...segments),
        stop: async () => {
