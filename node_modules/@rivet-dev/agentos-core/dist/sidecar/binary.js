...
export function resolvePublishedSidecarBinary() {
    ...
    if (override) {
        if (!existsSync(override)) {
            throw new Error(`AGENTOS_SIDECAR_BIN is set to ${override} but the file does not exist`);
        }
        return override;
    }
    ...
    let mod;
    try {
        mod = require("@rivet-dev/agentos-sidecar");
    }
    catch (error) {
        throw new Error("failed to resolve the Agent OS sidecar binary: the @rivet-dev/agentos-sidecar " +
            "package is not installed. Install it, or set AGENTOS_SIDECAR_BIN to a local " +
            `agentos-sidecar binary. (${error.message})`);
    }
